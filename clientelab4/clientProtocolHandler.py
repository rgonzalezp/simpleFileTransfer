...
class App:
    ...
    def __init__(self, master):
        ...
        self.quitbutton = Button(self.frame, text="QUIT", fg='red',
                                 command=self.frame.quit)
        self.quitbutton.grid(row=1, column=2)

        self.button = Button(self.frame, text="SEND", command=self.Send)
        ...
        self.folders = self.client.recv(1024).decode("utf-8")
        # First send to the server which folder to ask for
        gettext.insert(END, "Options: " + self.folders + "\n")
        ...
        if self.paso == 1:
            ...
            with open("repo/" + text, 'wb') as f:
                ...
                while pck:
                    ...
                    pck = self.client.recv(BUFFER_SIZE)
            self.sendtext.delete(0, END)
            self.gettext.insert(END, "Finished... Closed the connection. Bye! \n")
            self.gettext.configure(state='disabled')
# spawn the GUI
root = Tk()
root.title('Client')
